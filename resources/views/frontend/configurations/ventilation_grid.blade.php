    ...
    <div class="row">
        ...
        <div class="col-md-9 text-center">
            <ul class="steps">
                ...
                <li>9</li>
                <li>10</li>
                <li style="background: #ec4530"><b>11</b></li>
                @if($dti == 1) <li>12</li> @endif
            </ul>
            <hr>

            <h3>@lang('translations.ventilation_grid_choice'):</h3>

            <hr>


            <form action="{{route('classic_config_ventilation_grid_store')}}" method="post">
                {{csrf_field()}}
                ...
                @foreach($ventilationGrids as $ventilationGrid)
                    <div class="row">
                        <div class="thumbnail" style="border: solid thin #fd856e; border-radius: 15px;">
                            <img src="{{url($ventilationGrid->image_path)}}" alt="" style="max-height: 150px">
                            <div class="caption">
                                <input id="{{$ventilationGrid->id}}" type="radio" name="ventilation_grid" value="{{$ventilationGrid->id}}" required> <label for="{{$ventilationGrid->id}}">@if($language == 'sr'){{$ventilationGrid->name}} ( @elseif($language == 'en') {{$ventilationGrid->name_en}}( @endif  {{$ventilationGrid->price}} EUR )</label>
                            </div>
                        </div>
                    </div>
                @endforeach
